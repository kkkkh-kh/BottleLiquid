...
def build_sampler(dataset, args):
    if not args.balance_domains:
        return None

    weights = dataset.sample_weights(
        balance_label=True,
        balance_metadata_col=args.domain_col,
    )
...
def print_dataset_summary(name, dataset, domain_col=None):
    print(f"{name} samples: {len(dataset)}, class counts: {dataset.class_counts()}")
    if domain_col:
        print(f"{name} {domain_col} counts: {dataset.metadata_counts(domain_col)}")
